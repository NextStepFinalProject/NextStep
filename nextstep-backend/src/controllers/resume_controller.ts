...
const generateResume = async (req: Request, res: Response) => {
    try {
        const { feedback, jobDescription, templateName, userResumeFilename } = req.body;
        
        if (!feedback || !jobDescription || !templateName || !userResumeFilename) {
            return res.status(400).json({ error: 'Missing required fields' });
        }

        const resumePath = path.resolve(config.resources.resumesDirectoryPath(), userResumeFilename);

        const result = await generateImprovedResume(feedback, jobDescription, templateName, resumePath);
